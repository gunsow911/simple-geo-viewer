...
const useTuMeshVolume = (): UseMenuReturn => {
  const [asset, setAsset] = useState<DashboardAsset<TuMeshVolumeProp> | undefined>();
  const assetRef = useRef<DashboardAsset<TuMeshVolumeProp> | undefined>(undefined);
